...
export class UIManager extends Component {
  ...
  public setRemaining(remaining: number): void {
    if (this.remainLabel) {
      this.remainLabel.string = `剩余飞刃 ${remaining}`;
    }
  }
  ...
  private ensureRuntimeUI(): void {
    ...
    this.startPanel = this.startPanel ?? this.ensurePanel('StartPanel', '星际陨石', '启动挑战', 'StartButton', new Vec3(0, 0, 0));
    this.losePanel = this.losePanel ?? this.ensurePanel('LosePanel', '能量碰撞！', '重新挑战', 'RestartButton', new Vec3(0, 0, 0));
    this.winPanel = this.winPanel ?? this.ensurePanel('WinPanel', '星核突破！', '下一关', 'NextButton', new Vec3(0, 0, 0));
    this.ensureLoadingPanel();
    this.applyLoadingPanelBackground();
    this.applyStartPanelBackground();
    this.applyWinPanelBackground();
  }
  ...
  private ensureLoadingPanel(): void {
    ...
    this.applyNodeSprite(panel, 'meteor/ui_panel_meteor/spriteFrame');
    ...
    trackG.clear();
    trackG.fillColor = new Color(12, 18, 38, 230);
    trackG.roundRect(-250, -16, 500, 32, 16);
    trackG.fill();
    trackG.strokeColor = new Color(172, 84, 255, 190);
    ...
    fillG.clear();
    fillG.fillColor = new Color(196, 68, 255, 255);
    fillG.roundRect(0, -16, 500, 32, 16);
    fillG.fill();
    this.applyNodeSprite(track, 'meteor/ui_progress_bg_meteor/spriteFrame');
    this.applyNodeSprite(fill, 'meteor/ui_progress_fill_meteor/spriteFrame');
  ...
  private drawHudChip(g: Graphics, x: number, y: number, width: number): void {
    ...
    g.fillColor = new Color(13, 18, 40, 230);
    g.roundRect(x - halfWidth, y - 34, width, 68, 18);
    g.fill();

    g.fillColor = new Color(120, 52, 190, 72);
    g.roundRect(x - halfWidth + innerInset, y - 2, width - innerInset * 2, 30, 12);
    g.fill();

    g.strokeColor = new Color(146, 92, 255, 170);
    g.lineWidth = 1.5;
    g.roundRect(x - halfWidth, y - 34, width, 68, 18);
    g.stroke();
  }
  ...
  private ensurePanel(
    ...
    titleLabel.horizontalAlign = HorizontalTextAlignment.CENTER;
    titleLabel.color = new Color(225, 238, 255, 255);
    ...
    this.applyNodeSprite(btn, 'meteor/ui_button_meteor/spriteFrame');
    ...
    btnOutline.color = new Color(72, 24, 120, 230);
    btnOutline.width = 2;
  ...
  private drawButton(g: Graphics, width: number, height: number): void {
    ...
    g.fillColor = new Color(42, 28, 118, 255);
    g.roundRect(-width * 0.5, -height * 0.5, width, height, 18);
    g.fill();

    g.fillColor = new Color(92, 212, 255, 135);
    g.roundRect(-width * 0.5 + 10, 2, width - 20, height * 0.44, 14);
    g.fill();

    g.strokeColor = new Color(208, 118, 255, 230);
    g.lineWidth = 1.8;
    g.roundRect(-width * 0.5, -height * 0.5, width, height, 18);
    g.stroke();
  }
  ...
  private applyStartPanelBackground(): void {
    ...
    resources.load('meteor/bg_loading_meteor/spriteFrame', SpriteFrame, (err, sf) => {
  ...
  private applyLoadingPanelBackground(): void {
    if (!this._loadingPanel) {
      return;
    }

    this.applyPanelBackgroundImage(this._loadingPanel, 'BgImage', 'meteor/bg_loading_meteor/spriteFrame', 680, 560, 165);
  }

  private applyWinPanelBackground(): void {
    if (!this.winPanel) {
      return;
    }

    this.applyPanelBackgroundImage(this.winPanel, 'ClearBgImage', 'meteor/bg_clear_meteor/spriteFrame', 640, 430, 178);
  }

  private applyPanelBackgroundImage(panel: Node, childName: string, path: string, width: number, height: number, alpha: number): void {
    let bgNode = panel.getChildByName(childName);
    if (!bgNode) {
      bgNode = new Node(childName);
    ...
    resources.load(path, SpriteFrame, (err, sf) => {
      if (err || !sf || !bgNode.isValid || !sprite.isValid) {
        return;
      }
  ...
  private applyNodeSprite(node: Node, path: string): void {
    const sprite = node.getComponent(Sprite) ?? node.addComponent(Sprite);
    sprite.sizeMode = Sprite.SizeMode.CUSTOM;
    sprite.color = new Color(255, 255, 255, 255);

    resources.load(path, SpriteFrame, (err, sf) => {
      if (err || !sf || !node.isValid || !sprite.isValid) {
        return;
      }
      sprite.spriteFrame = sf;
