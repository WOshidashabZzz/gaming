...
| 文件 | 大小 |
| --- | ---: |
| `assets/resources/bg/bg_game_small.jpg` | 119.8 KB |
| `assets/resources/bg/bg_loading_small.jpg` | 36.9 KB |
...
| 文件 | 引用位置 |
| --- | --- |
| `assets/resources/bg/bg_game_small.jpg` | `GameManager.ts` 动态加载 `bg/bg_game_small/spriteFrame` |
| `assets/resources/bg/bg_loading_small.jpg` | `UIManager.ts` 动态加载 `bg/bg_loading_small/spriteFrame` |

## 已执行优化

| 资源 | 优化前 | 优化后 | 说明 |
| --- | ---: | ---: | --- |
| `bg_game_small.png` -> `bg_game_small.jpg` | 1083.3 KB | 119.8 KB | 转 JPG，尺寸调整为 720x1280 |
| `bg_loading_small.png` -> `bg_loading_small.jpg` | 347.7 KB | 36.9 KB | 转 JPG，尺寸调整为 720x484 |
...
assetManager.loadBundle('game-res', (err, bundle) => {
  ...
  bundle.load('bg/bg_game_small/spriteFrame', SpriteFrame, (loadErr, spriteFrame) => {
    // 使用分包资源
  });
});
```
